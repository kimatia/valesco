...
var fieldlimitertittle={

defaultoutput: "<b>[int]</b> characters remaining in title field.", //default message that gets output to statusid element
...
setup:function(config){
	if (config.thefield){ //if form field exists
		...
		this.addEvent([window], function(e){fieldlimitertittle.showlimit(config)}, "load")
		this.addEvent([window], function(e){fieldlimitertittle.cleanup(config)}, "unload")
		this.addEvent([config.thefield], function(e){return fieldlimitertittle.limitinput(e, config)}, "keypress")
		this.addEvent([config.thefield], function(){fieldlimitertittle.showlimit(config)}, "keyup")
	}
}
